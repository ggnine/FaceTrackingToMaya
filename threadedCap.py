    ...
    if 1==1 or int((time.time() - lastTime)*24)>4:
      ...
      try:
        ...
        for face in faces:
            ...
            if ready: # 0==1 and
                ready = False
                print("going out to",time.time())
                xx = threading.Thread(target=SendCommand, args=(list_of_face_points,))
                # t.daemon = True
                xx.start()
                # res = SendCommand(list_of_face_points)
